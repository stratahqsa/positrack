...
| `/insights` | **AI Insights** | A proactive, auto-generated briefing — top issues now, since-yesterday deltas, most-behind people. Read-only over `snapshot.ai_brief`, baked in upstream by the Snapshot job (3x/day: 8am/12pm/7pm IST); fail-soft (renders a graceful "unavailable" state, never an error) when generation didn't succeed this cycle. |
