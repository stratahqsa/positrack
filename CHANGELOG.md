...
- The OAuth connector no longer disconnects mid-session with "your connection to this connector was invalidated" after earlier calls in the same session succeeded. Because Hub's access tokens are opaque, the server verified the **id_token** on every call while FastMCP decided token refreshes from the **access token's** expiry — two different clocks. Once the id_token lapsed, validation failed but no refresh was triggered, so the call 401'd permanently until the user reconnected by hand; and since Hub need not return a new `id_token` when refreshing, the stored one could never be replaced, making Hub's id_token lifetime the real ceiling on a session regardless of the 30-day setting. Positrack now verifies the **access token** — the same one the tools forward to YouTrack and the one FastMCP refreshes — live against Hub's OIDC `userinfo` endpoint, so verification and refresh share one clock and the transparent refresh runs as designed. Successful verifications are cached for `OAUTH_USERINFO_CACHE_SECONDS` (default 5 min), which also tightens revocation from "within one Hub access-token lifetime" to that window; only a 401/403 from Hub counts as a rejection, so a Hub timeout, 404 or 5xx lets an already-verified session continue for up to `OAUTH_USERINFO_GRACE_SECONDS` (default 1 hour) rather than logging everyone out, while an unverifiable new session still fails closed. Set `OAUTH_VERIFY_VIA_USERINFO=0` to revert to the previous id_token behaviour; if Hub ever advertises no `userinfo_endpoint` the server falls back to it automatically rather than guessing a URL.
- OAuth connector sessions no longer expire roughly once a day. FastMCP was mirroring Hub's own access-token `expires_in` in the token it issued to the connector, so every user was pushed through a full browser re-auth each morning; the issued token now lasts `OAUTH_ACCESS_TOKEN_TTL_SECONDS` (default 30 days) and the Hub token is transparently refreshed behind it, with a `OAUTH_TOKEN_REFRESH_LEEWAY_SECONDS` (default 60s) margin so a call can't race an expiring token. This can't grant access beyond Hub: a revoked session stops working within one Hub access-token lifetime (validation is a local JWKS check rather than a live call to Hub, so revocation lands when the held token lapses and its renewal is refused — a window Hub sets, which the 30 days does not widen), and if Hub issues no refresh token the lifetime is still capped at Hub's `expires_in`.
- The OAuth store holds the upstream Hub access/refresh tokens as well as DCR client registrations, so an unmounted volume logged **every** connected user out on each redeploy, not just broke "Client Not Registered". The no-volume warning now says so, and the store can be encrypted at rest via `OAUTH_STORE_ENCRYPTION_KEY` (a Fernet key) — previously tokens were written to the volume in plaintext, a downgrade from FastMCP's encrypted default. Both fall back safely: a bad key or failed wrapper keeps persistence rather than losing it.
